...
test('all technology branches are acyclic and reachable through their stated prerequisites',()=>{
  const learned=new Set();for(let i=0;i<10;i++)for(const [id,t]of Object.entries(TECHNOLOGIES))if(t.requires.every(id=>learned.has(id)))learned.add(id);assert.equal(learned.size,Object.keys(TECHNOLOGIES).length);assert.equal(new Set(Object.values(TECHNOLOGIES).map(t=>t.branch)).size,3);
});
