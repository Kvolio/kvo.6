...
try{for(const spec of [{name:'desktop',width:1440,height:900,touch:false},{name:'phone-portrait',width:390,height:844,touch:true},{name:'phone-small',width:320,height:568,touch:true},{name:'phone-landscape',width:844,height:390,touch:true},{name:'tablet',width:1024,height:768,touch:true}]){
 const context=await browser.newContext({viewport:{width:spec.width,height:spec.height},hasTouch:spec.touch,isMobile:spec.touch,deviceScaleFactor:spec.touch?2:1});const page=await context.newPage();const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:4173/?test');await page.locator('#new-game').click();await page.waitForTimeout(250);
 ...
 await page.locator('[data-build="farm"]').click();const site=await page.evaluate(()=>{const app=window.__tidehold;app.game.paused=true;return app.renderer.worldToScreen(35.5*40,26.5*40);});const box=await page.locator('#game').boundingBox();
