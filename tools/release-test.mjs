...
try{
 ...
 await page.goto('http://127.0.0.1:4173/dist/index.html?test');await page.locator('#new-game').click();await page.locator('[data-tab="army"]').click();await page.locator('[data-action="select-workers"]').click();await page.locator('[data-action="move"]').click();
 ...
 const context=await browser.newContext({offline:true});const filePage=await context.newPage();filePage.on('pageerror',e=>errors.push(e.message));await filePage.goto(pathToFileURL(path.resolve('dist/index.html')).href);await filePage.locator('#new-game').click();assert.equal(await filePage.locator('#modal').isVisible(),false);assert.ok(await filePage.locator('#game').isVisible());assert.equal(await filePage.locator('[data-build="farm"]').count(),1);assert.deepEqual(errors,[]);
