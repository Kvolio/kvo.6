import {BUILDINGS,UNITS,RESOURCES,ENEMIES,KEEP_PERKS} from './data.js';
...
export class UI {
  constructor(game,renderer,input,saves,audio,onGame,onTitle){
    ...
    game.autosave=()=>{if(!saves.save(game)&&!this.saveWarning){this.saveWarning=true;this.toast('Progress could not be saved. Keep this tab open and try Save again.');}};
  ...
  buildReason(type){const g=this.game,d=BUILDINGS[type];if(type==='temple'&&g.buildings.some(b=>b.type==='temple'&&b.hp>0))return 'Only one Temple per kingdom';return g.level<(d.level||1)?`Requires Keep level ${d.level}`:!g.afford(d.cost)?'Not enough resources':'';}
  ...
  renderPanel(force=false){
    ...
    if(this.tab==='build'||this.tab==='army'){
      ...
      if(force||this.panelKey!==key){this.panelKey=key;
        if(this.tab==='build'){
          ...
          $('panel').innerHTML=`<div class="army-actions"><button data-action="select-army">Select army</button><button data-action="select-workers">Select workers</button><button data-action="research">Research tree</button></div><div class="category army-categories">${['All','Workers','Infantry','Ranged','Cavalry','Support'].map(name=>`<button data-army-category="${name}" class="${this.armyCategory===name?'active':''}">${name}</button>`).join('')}</div><div class="build-list">${Object.entries(UNITS).filter(([type])=>this.armyCategory==='All'||({worker:'Workers',militia:'Infantry',spear:'Infantry',archer:'Ranged',crossbow:'Ranged',knight:'Infantry',mountedknight:'Cavalry',mountedarcher:'Cavalry',cleric:'Support'})[type]===this.armyCategory).map(([type,d])=>{const reason=this.recruitReason(type);return `<button class="build-card" data-recruit="${type}" aria-disabled="${!!reason}"><img src="${this.renderer.unitArt.thumbnail(type)}" alt=""><span class="copy"><strong>${d.name}</strong><small class="${reason?'unavailable':''}">${reason||(d.healer?`${d.hp} HP · heals 3 allies · ${TRAINING_SECONDS[type]}s training`:`${d.hp} HP · ${d.damage} damage · ${TRAINING_SECONDS[type]}s training`)}</small><span class="cost">${costText(d.cost)}</span></span></button>`;}).join('')}</div>`;
        }
      }
    }
    ...
    if(force||this.inspectKey!==key){this.inspectKey=key;
      if(b){const d=BUILDINGS[b.type];
        $('inspection').innerHTML=`<div class="inspect-hero"><img src="${this.renderer.art.thumbnail(b.type,b.level,b.type==='keep'?g.keepDesign:0)}" alt="${d.name} roof"><div><span class="eyebrow">${b.complete?'LEVEL '+b.level:'CONSTRUCTION'}</span><h2 class="inspect-title">${d.name}</h2></div></div><div class="details"><div class="stat-line"><span>Condition</span><strong id="selected-hp"></strong></div><progress id="selected-progress" max="1"></progress><div id="worker-detail"></div><div id="upgrade-detail"></div><div id="tower-detail"></div><div id="training-detail"></div><p>${d.desc}${b.type==='keep'?' '+g.keepPerk.description:b.type==='temple'?' Clerics: '+g.units.filter(u=>u.type==='cleric').length+' / '+g.clericCap+'.':''}</p></div><div class="action-list">${d.workers||!b.complete?'<button data-action="assign">Assign worker</button><button data-action="unassign">Remove worker</button>':''}${b.complete?'<button data-action="repair">Repair</button>':''}${b.complete&&!b.upgrade&&b.level<5?`<button data-action="upgrade">Upgrade · E</button><span class="cost wide">Upgrade: ${costText({wood:100*b.level,stone:100*b.level,gold:50*b.level})}</span>`:''}${b.upgrade?'<button class="wide" data-action="cancel-upgrade">Cancel upgrade · recover 80%</button>':''}${g.isWallPlatform(b)?'<button class="wide" data-action="wall-defenders">Select wall defenders · '+g.wallOccupants(b).length+'/'+g.wallCapacity(b)+'</button>':''}${d.damage&&!b.upgrade?'<button data-action="range-upgrade">Improve range</button><button data-action="damage-upgrade">Improve damage</button>':''}${b.type==='keep'?'<button class="wide" data-recruit="worker">Train worker · 30 food</button>':''}${['smith','temple'].includes(b.type)?'<button class="wide" data-action="research">Open research tree</button>':''}${b.type!=='keep'?`<button class="wide danger" data-action="demolish">${b.complete?'Demolish building':'Cancel construction'}</button>`:''}</div>`;
    ...
    if($('unit-details'))$('unit-details').textContent=units.length===1?`HP ${Math.ceil(units[0].hp)} / ${units[0].maxHp} · ${UNITS[units[0].type].damage} damage · ${units[0].kills} kills · ${units[0].garrison?'Wall cover · 55% damage reduction · +1.5 tiles range':units[0].mountOrder?'Moving onto wall':units[0].job?.kind||units[0].stance}`:'Choose an order below, then a destination on the map.';
    if(units.length===1&&units[0].type==='cleric'&&$('unit-details'))$('unit-details').textContent='HP '+Math.ceil(units[0].hp)+' / '+units[0].maxHp+' · Heals 3 allies for '+(8*(1+g.researchBonus('healing'))).toFixed(1)+' HP each second within 5.25 tiles.'+(g.researched.includes('massheal')?' Mass Heal: all allies every 12 seconds.':'')+(g.researchBonus('blessing')?' Aura: +'+(g.researchBonus('blessing')*5)+'% damage, +'+g.researchBonus('blessing')+' armor.':'');
    if(tile&&$('deposit-remaining'))$('deposit-remaining').textContent=`${Math.ceil(tile.amount)} ${tile.resource||''}`;
  }
  renderResearch(force=false){
    const g=this.game,source=this.researchBuilding||'smith',task=g.researchTask,key=[source,g.researched.join(','),task?.id,task?.waiting,...Object.keys(TECHNOLOGIES).map(id=>g.researchReason(id))].join('|');
    if(force||this.researchKey!==key){this.researchKey=key;$('tray-title').textContent=BUILDINGS[source].name+' · Research tree';
      $('panel').innerHTML='<div id="research-progress"></div><div class="tech-tree">'+(source==='temple'?['Healing','Blessings']:['Economy','Army','Defenses']).map(branch=>'<section class="tech-branch"><h3>'+branch+'</h3>'+Object.entries(TECHNOLOGIES).filter(([,t])=>t.branch===branch&&t.building===source).map(([id,t])=>{const done=g.researched.includes(id),active=task?.id===id,reason=g.researchReason(id);return '<button class="tech-card '+(done?'researched':active?'researching':'')+'" data-research="'+id+'" aria-disabled="'+!!reason+'"><small>'+(BUILDINGS[t.building].name+' level '+t.tier+' · ')+(t.requires.length?'Requires: '+t.requires.map(id=>TECHNOLOGIES[id].name).join(' + '):'Foundation')+'</small><strong>'+t.name+'</strong><span>'+t.description+'</span><small class="cost">'+(done?'✓ Learned':active?'Researching…':costText(t.cost)+' · '+t.time+'s')+'</small>'+(!done&&!active&&reason?'<small class="unavailable">'+reason+'</small>':'')+'</button>';}).join('')+'</section>').join('')+'</div>';
    }
  ...
  newKingdom(){
    this.dialog(`<div><span class="eyebrow">YOUR STORY STARTS HERE</span><h1>A new<br>kingdom.</h1><p>One Keep and five workers. New mountain ranges, new opportunities. Starting a new kingdom replaces the saved run when the next save occurs.</p><label for="kingdom-name">Name your kingdom</label><input id="kingdom-name" maxlength="32" value="Tidehold" autocomplete="off"><label>Choose your Keep</label><div class="keep-choices">${KEEP_PERKS.map(({name,description},i)=>`<button data-keep-design="${i}" aria-pressed="${i===this.newKeepDesign}" ${i===3&&!this.saves.demonUnlocked()?'disabled':''}><img src="${this.renderer.art.thumbnail('keep',1,i)}" alt=""><span>${name}</span><small>${i===3&&!this.saves.demonUnlocked()?'Beat Nightmare to unlock':description}</small></button>`).join('')}</div><label for="difficulty">Choose your challenge</label><select id="difficulty"><option value="normal">Normal · The intended challenge</option><option value="easy">Easy · 40 waves · A gentler beginning</option><option value="hard">Hard · Invasions around the island</option><option value="nightmare">Nightmare · for experienced players, the broken seal</option></select><div class="modal-actions"><button id="new-game" class="primary">Found a kingdom →</button><button id="new-back">Back</button></div></div>${this.titleArt(this.newKeepDesign,1)}`,true);
    for(const el of document.querySelectorAll('[data-keep-design]'))el.onclick=()=>{if(el.disabled)return;this.newKeepDesign=Number(el.dataset.keepDesign);for(const button of document.querySelectorAll('[data-keep-design]'))button.setAttribute('aria-pressed',String(button===el));document.querySelector('.title-art img').src=this.renderer.art.thumbnail('keep',1,this.newKeepDesign);};
