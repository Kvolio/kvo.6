...
export class UI {
  ...
  update(){
    ...
    $('wave-state').textContent=g.active?(g.sealBroken?'The demon legion attacks. Defend the Keep.':'Defend the Keep. Hold the coast.'):g.wave?'Repair, gather, and prepare.':'The sea is quiet. For now.';
    ...
    const boss=g.enemies.find(e=>ENEMIES[e.type].boss);$('boss').hidden=!boss;$('viewport').classList.toggle('has-boss',!!boss);if(boss)$('boss').innerHTML=`${ENEMIES[boss.type].name}<small>${g.isAirborne(boss)?'AIRBORNE · RANGED ONLY · ':''}${boss.phase==='healing'?'REBIRTH · '+Math.ceil(boss.healRemaining)+'s':boss.phase===2?'PHASE II · KEEP ASSAULT':boss.castUntil>g.time?boss.casting.toUpperCase():Math.round((ENEMIES[boss.type].defense||0)*100)+'% DEFENSE'}</small><progress max="${boss.maxHp}" value="${boss.hp}"></progress>`;
