...
export class Game {
 constructor(seed=Date.now(),difficulty='normal'){
  this.world=new World(seed);this.difficulty=difficulty;this.resources={wood:250,stone:150,food:150,iron:0,gold:100};if(difficulty==='easy')for(const k in this.resources)this.resources[k]*=1.5;
 ...
 place(type,tx,ty){const d=BUILDINGS[type];if(this.over||!d||type==='keep')return false;if((d.level||1)>this.level){this.notice(`Requires Keep level ${d.level}.`);return false;}if(!this.world.valid(type,tx,ty,this.buildings)){this.notice(d.resource&&d.resource!=='food'?`Find clear land near ${d.resource}.`:'Choose clear land, away from water and buildings.');return false;}if(!this.spend(d.cost)){this.notice('You need more resources.');return false;}
 ...
 recruit(type){const d=UNITS[type];if(this.over||!d)return false;const b=this.buildings.find(b=>b.type===d.building&&b.complete&&b.hp>0);if(!b||(d.level||1)>this.level){this.notice(`Requires ${BUILDINGS[d.building].name}${d.level?` and Keep level ${d.level}`:''}.`);return false;}if(this.units.length>=this.capacity){this.notice('Build housing to increase population.');return false;}if(!this.spend(d.cost)){this.notice('Not enough resources to recruit.');return false;}
  let pos={x:b.x,y:(b.ty+BUILDINGS[b.type].size+.5)*TILE};const free=this.world.tiles.find(t=>Math.hypot(t.x+.5-pos.x/TILE,t.y+.5-pos.y/TILE)<3&&t.type!=='water'&&!this.world.buildingAt(this.buildings,t.x,t.y));if(free)pos={x:(free.x+.5)*TILE,y:(free.y+.5)*TILE};this.addUnit(type,pos.x,pos.y);this.notice(`${d.name} ready.`);return true;
 }
 assign(u,b,kind){if(!u||u.type!=='worker')return;u.job={building:b.id,kind,stage:'travel',clock:0,carry:0};u.path=[];u.target=null;}
 assignWorker(b){if(!b)return;const d=BUILDINGS[b.type];const workers=this.units.filter(u=>u.job?.building===b.id);if(b.complete&&(!d.workers||workers.length>=d.workers)){this.notice('No available worker slots.');return;}const u=this.units.find(u=>u.type==='worker'&&!u.job);if(!u){this.notice('No idle workers. Train one at the Keep.');return;}this.assign(u,b,b.complete?'gather':'build');this.notice('Worker assigned.');}
 ...
 command(point,stance='move'){if(this.world.at(point.x,point.y)?.type==='water'){this.notice('Your troops cannot cross the sea.');return;}let i=0;for(const u of this.units.filter(u=>this.selected.includes(u.id))){u.job=null;u.target={x:clamp(point.x+(i%4)*16,20,COLS*TILE-20),y:clamp(point.y+Math.floor(i/4)*16,20,ROWS*TILE-20)};u.path=[];u.pathRevision=-1;u.stance=stance;i++;}if(i)this.effects.push({x:point.x,y:point.y,kind:'command',life:1,max:1});}
 move(u,target,dt,enemy=false){if(distance(u,target)<12)return true;const key=`${Math.floor(target.x/TILE)},${Math.floor(target.y/TILE)}`;
  ...
  if(!u.path.length)return distance(u,target)<TILE*1.7;
 ...
 worker(u,dt){const job=u.job;if(!job){if(u.target&&this.move(u,u.target,dt))u.target=null;return;}const b=this.buildings.find(b=>b.id===job.building&&b.hp>0);if(!b){u.job=null;return;}
  if(job.kind==='build'||job.kind==='repair'){
   const goal={x:b.x,y:(b.ty+BUILDINGS[b.type].size+.5)*TILE};if(!this.move(u,goal,dt))return;
  ...
  if(job.stage==='return'){
   const drops=this.buildings.filter(b=>b.complete&&(b.type==='keep'||b.type==='warehouse'));const drop=drops.sort((a,b)=>distance(u,a)-distance(u,b))[0];if(!drop)return;
   if(this.move(u,{x:drop.x,y:(drop.ty+BUILDINGS[drop.type].size+.5)*TILE},dt)){const amount=Math.min(job.carry,this.storage-this.resources[resource]);this.resources[resource]+=amount;this.stats.gathered+=amount;job.carry=0;job.stage='travel';}return;
  }
  let tile=job.tile!=null?this.world.tiles[job.tile]:null;if(resource!=='food'&&(!tile||tile.amount<=0)){
   tile=this.world.tiles.filter(t=>t.resource===resource&&t.amount>0&&Math.hypot(t.x-b.tx,t.y-b.ty)<8).sort((a,c)=>Math.hypot(a.x-b.tx,a.y-b.ty)-Math.hypot(c.x-b.tx,c.y-b.ty))[0];if(!tile)return;job.tile=tile.y*COLS+tile.x;
  }
  const point=resource==='food'?{x:b.x,y:(b.ty+2.5)*TILE}:{x:(tile.x+.5)*TILE,y:(tile.y+.5)*TILE};if(!this.move(u,point,dt))return;job.clock+=dt;
