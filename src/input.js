...
export class Input {
 ...
 up(e){const p=this.point(e),start=this.start;this.pointers.delete(e.pointerId);if(this.gesture){if(!this.pointers.size){this.gesture=false;this.start=null;}return;}if(!start)return;this.start=null;
  ...
  g.selectedTile=null;if(target)g.selected=start.shift?[...new Set([...g.selected,target.id])]:[target.id];else{g.selected=[];const tile=g.world.at(w.x,w.y);if(tile?.resource)g.selectedTile=tile.y*64+tile.x;}this.onSelect();
 }
