...
export class World {
  constructor(seed=Date.now(),savedTiles=null) {
    this.seed=seed; this.tiles=[]; this.reachCache=new Map();
    ...
    for(const t of this.tiles){
      ...
      else t.resource=n<.11+(Math.sin(t.x*.3)*Math.cos(t.y*.24)+1)*.07?'wood':n>.955?'stone':n>.94?'iron':n>.932?'gold':null;
  ...
  tile(x,y){if(x<0||y<0||x>=COLS||y>=ROWS)return null;return this.tiles[Math.floor(y)*COLS+Math.floor(x)];}
  ...
  neighbors(id){const x=id%COLS,y=Math.floor(id/COLS);return [x>0?id-1:-1,x<COLS-1?id+1:-1,y>0?id-COLS:-1,y<ROWS-1?id+COLS:-1].filter(n=>n>=0);}
  ...
  connectLand(){
    let connected=this.flood(28*COLS+32);
    while(true){
      const target=this.tiles.findIndex((t,id)=>this.walkable(t)&&!connected.has(id));if(target<0)break;
      const prev=new Int32Array(COLS*ROWS).fill(-1),queue=[target];prev[target]=target;let end=-1;
      ...
      connected=this.flood(28*COLS+32);
    }
  }
  ...
  blocked(buildings,enemy=false){
    ...
    for(const b of buildings){if(b.hp<=0||b.type==='road'||(!enemy&&b.type==='gate'))continue;const size=BUILDINGS[b.type].size;
      for(let y=b.ty;y<b.ty+size;y++)for(let x=b.tx;x<b.tx+size;x++)result.add(y*COLS+x);
    }
    return result;
  }
  ...
  placementReason(type,tx,ty,buildings){
    ...
    if(!d||!Number.isInteger(tx)||!Number.isInteger(ty)||tx<0||ty<0||tx+d.size>COLS||ty+d.size>ROWS)return 'Choose a site inside the kingdom.';
  ...
  path(from,to,buildings,{enemy=false,flying=false}={}){
    ...
    const start=startTile.y*COLS+startTile.x,goal=goalTile.y*COLS+goalTile.x,blocked=this.blocked(buildings,enemy);
    const search=breakWalls=>{
      const prev=new Int32Array(COLS*ROWS).fill(-1),queue=[start];prev[start]=start;let found=-1;
      for(let head=0;head<queue.length;head++){
        const id=queue[head];if(id===goal){found=id;break;}
        for(const next of this.neighbors(id)){
          if(prev[next]!==-1||!this.walkable(this.tiles[next])||(!breakWalls&&blocked.has(next)))continue;
          prev[next]=id;queue.push(next);
        }
      }
      if(found<0)return null;const result=[];
      for(let id=found;id!==start;id=prev[id])result.push({x:(id%COLS+.5)*TILE,y:(Math.floor(id/COLS)+.5)*TILE});
